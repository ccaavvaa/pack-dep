...
class Upgrade {
    ...
    async executePublish(pu, plan) {
        ...
        if (plan.repositoryUrl) {
            await spinner_1.spinner(`Set npm repository ${plan.repositoryUrl}`, async () => {
    ...
    async executeUpdateFolder(pu, plan) {
        const packageFolder = pu.folder;
        let stat;
        try {
